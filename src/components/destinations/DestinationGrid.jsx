...
/**
 * DestinationGrid - Reusable grid component for displaying destinations
 * Uses unified DestinationCard with proper memoization
 */
function DestinationGrid({ destinations, category, className = '' }) {
    ...
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 ${className}`}>
      {destinations.map((destination) => (
        <DestinationCard
          ...
          showDuration={!!destination.duration}
        />
      ))}
    </div>
  );
